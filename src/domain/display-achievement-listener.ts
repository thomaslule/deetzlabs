...
export function displayAchievementListener(
  query: Query,
  showAchievement: ShowAchievementFunc,
  options: Options
) {
  return async (event: Event) => {
    if (
      event.aggregate === "viewer" &&
      (event.type === "got-achievement" ||
        event.type === "replayed-achievement")
    ) {
      const achievement = options.achievements[event.achievement];
      const viewer = await query.getViewer(event.id);
      if (viewer === undefined) {
        throw new Error(`could not find viewer ${event.id}`);
      }
