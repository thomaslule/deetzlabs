...
export class ViewerDomain {
  ...
  private async rebuildDecisionFor(id: string) {
    try {
      ...
      const projection = await projectFromEvents(
        makeDecisionReducer(getDecisionReducer(this.options)),
        events
      );
      await this.storage
        .getKeyValueStorage("viewer-decision")
        .store(id, projection);
    } catch (err) {
      logger.error(
        "could not rebuild decision projection for viewer %s: %s",
        id,
        err
      );
    }
