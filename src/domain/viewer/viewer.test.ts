...
describe("Viewer", () => {
  ...
  test("it should be able to distribute 2 achievements on the same event", async () => {
    ...
    const achievementFollow = {
      ...
      text: "",
      description: "",
      distributeWhen: (state: undefined, event: Event) =>
        event.type === "followed",
    };
    const options = {
      ...testOptions,
    ...
    expect(publish).toHaveBeenCalledWith(
      { ...viewerEvent, type: "got-achievement", achievement: "test1" },
      expect.anything()
    );
    expect(publish).toHaveBeenCalledWith(
      { ...viewerEvent, type: "got-achievement", achievement: "test2" },
      expect.anything()
    );
  });
  ...
  describe("cheer", () => {
    test("it should throw if amount is not a number or a number <= 0", async () => {
      ...
      await expect(
        someone.cheer("12" as any as number, "hi")
      ).rejects.toThrow();
    });
    ...
    test("it should publish deserved achievements", async () => {
      ...
      expect(publish).toHaveBeenCalledWith(
        { ...viewerEvent, type: "got-achievement", achievement: "cheerleader" },
        expect.anything()
      );
    });
    ...
    test("it shouldnt publish already obtained achievements", async () => {
      const publish = jest.fn().mockImplementation((event) => event);
      const someone = getViewer(publish, {
        achievementsReceived: ["cheerleader"],
      });
