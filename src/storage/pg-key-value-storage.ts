...
export class PgKeyValueStorage<T> implements KeyValueStorage<T> {
  ...
  public async get(key: string): Promise<T | undefined> {
    ...
    const result = await this.db.query(
      "select value from projections where name = $1 and key = $2",
      [this.name, key]
    );
  ...
  public async delete(key: string) {
    await this.db.query(
      "delete from projections where name = $1 and key = $2",
      [this.name, key]
    );
  }
  ...
  public async getAll(): Promise<Dictionary<T>> {
    const result = await this.db.query(
      "select key, value from projections where name = $1",
      [this.name]
    );
    return result.rows.reduce(
      (acc, row) => ({ ...acc, [row.key]: row.value.value }),
      {}
    );
  }
