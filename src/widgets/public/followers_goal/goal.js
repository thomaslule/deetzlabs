function refreshAmount(amount, goal) {
  ...
  document.getElementById('current_bar').style.width = `${Number((100 * amount) / goal).toFixed(2)}%`;
}

function refresh(goal) {
  fetch(`https://api.twitch.tv/kraken/channels/${window.config.channel}/follows?limit=1`, {
    method: 'GET',
    headers: {
